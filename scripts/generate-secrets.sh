...
stringData:
  ...
  admin-password: \"${admin_password}\""
    ...
    # Save plaintext for credential vault
    VAULT_FARADAY_ADMIN_PASSWORD="$admin_password"
...
stringData:
  ...
  postgres-password: \"${db_password}\""
    ...
    # Save plaintext for credential vault
    VAULT_OPENVAS_ADMIN_PASSWORD="$admin_password"
...
stringData:
  users: \"${users_entry}\""
    ...
    # Save plaintext for credential vault
    VAULT_TRAEFIK_ADMIN_PASSWORD="$admin_password"
...
stringData:
  ...
  admin.passwordMtime: \"$(date -u +%Y-%m-%dT%H:%M:%SZ)\""
    ...
    # Save plaintext for credential vault
    VAULT_ARGOCD_ADMIN_PASSWORD="$admin_password"
...
stringData:
  ...
  admin-password-hash: \"${admin_hash}\""
    ...
    # Save plaintext for credential vault
    VAULT_SCANNING_CONSOLE_ADMIN_PASSWORD="$admin_password"
...
generate_portal_secrets() {
    ...
    # Reuse the same SECRET_KEY for cross-app SSO
    local secret_key="${SHARED_SECRET_KEY:-$(generate_password 48)}"

    # Portal uses hardcoded admin/admin from talos_common default — no password hash needed.
    # Users retrieve all other service passwords from the portal's Credentials page.

    local content="# Unified Portal Credentials
# Generated by generate-secrets.sh - DO NOT COMMIT UNENCRYPTED
# NOTE: Portal uses hardcoded admin/admin (talos_common default).
# No admin-password-hash — app falls back to built-in default.
---
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  secret-key: \"${secret_key}\""
    ...
    print_info "  Admin Password: admin (hardcoded default)"
...
#######################################
# Generate credential vault for portal UI
# Writes all service credentials as a K8s
# Secret mounted into the portal pod.
#######################################
generate_credential_vault() {
    print_info "Generating credential vault for Portal..."

    local credentials_yaml
    credentials_yaml=$(cat <<CRED_EOF
- service: ArgoCD
  url: https://argocd.knowledgeondemand.net
  username: admin
  password: "${VAULT_ARGOCD_ADMIN_PASSWORD}"
- service: OpenVAS
  url: https://openvas.knowledgeondemand.net
  username: admin
  password: "${VAULT_OPENVAS_ADMIN_PASSWORD}"
- service: Faraday
  url: https://faraday.knowledgeondemand.net
  username: admin
  password: "${VAULT_FARADAY_ADMIN_PASSWORD}"
- service: Harbor
  url: https://harbor.knowledgeondemand.net
  username: admin
  password: Harbor12345
- service: Scanning Console
  url: https://scan.knowledgeondemand.net
  username: admin
  password: "${VAULT_SCANNING_CONSOLE_ADMIN_PASSWORD}"
...
CRED_EOF
    ...
    local content="# Credential Vault — mounted into Portal for the Credentials page
# Generated by generate-secrets.sh - DO NOT COMMIT UNENCRYPTED
---
apiVersion: v1
kind: Secret
...
stringData:
  credentials.yaml: |
$(echo "$credentials_yaml" | sed 's/^/    /')"

    write_encrypted_secret "${PROJECTS_DIR}/portal/credential-vault.sops.yaml" "$content"

    print_info "  Vault contains credentials for $(echo "$credentials_yaml" | grep -c '^\- service:') services"
}
...
# Credential vault accumulators (plaintext passwords for portal retrieval page)
VAULT_ARGOCD_ADMIN_PASSWORD=""
VAULT_FARADAY_ADMIN_PASSWORD=""
VAULT_OPENVAS_ADMIN_PASSWORD=""
VAULT_TRAEFIK_ADMIN_PASSWORD=""
VAULT_SCANNING_CONSOLE_ADMIN_PASSWORD=""
