...
# =============================================================================
# WebSocket Connection Manager (scan-specific)
# =============================================================================
...
def create_scan_message(msg_type: str, data: dict) -> dict:
    return {
        "type": msg_type,
...
async def scan_log_callback(entry: ScanLogEntry):
    ...
    message = create_scan_message("scan_log", {
        "tool": entry.tool,
        "level": entry.level,
...
async def scan_tool_callback(tool_state: ScanToolState):
    ...
    message = create_scan_message("scan_tool_update", {
...
async def scan_websocket(
    ...
    try:
        ...
        scan = service.get_status()
        if scan:
            await websocket.send_json(create_scan_message("scan_initial_state", {
                "id": scan.id,
                "target": scan.target,
                "profile": scan.profile.value,
                "status": scan.status.value,
