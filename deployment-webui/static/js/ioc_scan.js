...
function iocScanManager() {
    return {
        ...
        async pollUpdates() {
            try {
                const statusResp = await fetch('/api/ioc-scan/status');
                ...
                // Fetch new logs
                const logResp = await fetch(`/api/ioc-scan/logs?offset=${this.logs.length}`);
                ...
                if (prevStatus !== 'idle' && !statusData.is_running && this.isRunningStatus(prevStatus)) {
                    this.stopPolling();
                    this.stopTimer();
                    await this.fetchHistory();
                }
        ...
        connectWebSocket() {
            ...
            const url = `${protocol}//${window.location.host}/api/ioc-scan/ws?token=${token}`;
            ...
            try {
                ...
                this.ws.onmessage = (event) => {
                    try {
                        ...
                        console.error('Failed to parse IOC scan WebSocket message:', e);
                    }
                };
            } catch (e) {
                console.error('Failed to create IOC scan WebSocket:', e);
                setTimeout(() => this.connectWebSocket(), 3000);
            }
        },
        ...
        async startScan() {
            ...
            try {
                const response = await fetch('/api/ioc-scan/start', {
                    method: 'POST',
        ...
        async abortScan() {
            ...
            try {
                const response = await fetch('/api/ioc-scan/abort', { method: 'POST' });
                if (response.ok) {
                    this.status = 'aborted';
                    this.stopTimer();
                    this.stopPolling();
        ...
        async fetchStatus() {
            try {
                const response = await fetch('/api/ioc-scan/status');
        ...
        async fetchHistory() {
            try {
                const response = await fetch('/api/ioc-scan/history');
                const data = await response.json();
        ...
        updateElapsed() {
            ...
            const s = diff % 60;
            this.elapsedTime = `${m}:${s.toString().padStart(2, '0')}`;
        },
