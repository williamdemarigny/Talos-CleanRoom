...
class WebSocketBase {
    ...
    constructor(wsPath, pollStatusUrl, pollLogsUrl, options = {}) {
        ...
        // Zombie WebSocket detection: tracks whether a pong was received
    ...
    async pollUpdates() {
        ...
        try {
            ...
            if (this._consecutivePollErrors <= 3) {
                console.warn('Polling error (' + this._consecutivePollErrors + '/3):', e.message || e);
            }
            if (this._consecutivePollErrors >= 10) {
                console.error('Polling failed 10 times consecutively, stopping.');
                this.stopPolling();
            }
        }
    }
        ...
        try {
            ...
            comp.ws.onmessage = (event) => {
                try {
                    ...
                    // Handle pong for zombie detection
    ...
    // =================================================================
    // Heartbeat Ping with Zombie Detection
    // =================================================================

    /**
     * Start sending periodic ping messages over the WebSocket.
     * If a pong is not received before the next ping, the connection is
     * considered dead and will be forcefully reconnected.
     *
    ...
    /**
     * Reset the log poll offset (call when logs are cleared, e.g. new run).
     */
    resetLogOffset() {
        this._logOffset = 0;
    }
    ...
    addLog(data) {
        ...
        // Keep log buffer manageable — adjust offset to stay in sync
