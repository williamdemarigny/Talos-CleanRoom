...
class DeploymentTUI:
    ...
    def cleanup(self):
        ...
        if self.repo_root:
            tf_dir = self.repo_root / "Resources" / "IAC-DNS" / "terraform" / "talos-cluster-create"
            if tf_dir.exists():
    ...
    def step_2_talos_config_generation(self) -> bool:
        ...
        iac_dir = self.repo_root / "Resources" / "IAC-DNS"
        talos_dir = iac_dir / "talos"

        # Run tfvars-to-talos-env.sh
        console.print("  [cyan]Running tfvars-to-talos-env.sh...[/cyan]")
        script_path = iac_dir / "tfvars-to-talos-env.sh"
        ...
        success, _ = self.run_command(["bash", str(script_path), "--backup"], cwd=iac_dir)
        if not success:
    ...
    def step_3_apply_talos_configs(self) -> bool:
        ...
        talos_dir = self.repo_root / "Resources" / "IAC-DNS" / "talos"
    ...
    def step_6_install_argocd(self) -> bool:
        ...
        argocd_dir = self.repo_root / "Resources" / "IAC-DNS" / "infrastructure" / "argocd"
    ...
    def step_7_deploy_infrastructure(self) -> bool:
        ...
        projects_dir = self.repo_root / "Resources" / "IAC-DNS" / "infrastructure" / "projects"
