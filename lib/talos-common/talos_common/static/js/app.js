...
// Logout function — uses OIDC logout if available, otherwise local logout
async function logout() {
    try {
        localStorage.removeItem('access_token');
        // Try OIDC logout first (redirects to Keycloak end-session)
...
async function authFetch(url, options = {}) {
    ...
    if (response.status === 401) {
        // Redirect to OIDC login if available, fallback to local login
