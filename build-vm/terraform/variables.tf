...
variable "lxc_tags" {
  ...
  type        = list(string)
  default     = ["build", "docker", "management"]
}
...
variable "lxc_ip_address" {
  description = "IP address for the container in CIDR notation (e.g., 10.83.3.191/24)."
  type        = string
}
...
variable "ssh_user_groups" {
  description = "Groups for the SSH user (comma-separated). Includes docker for container builds."
  type        = string
  default     = "sudo,docker"
}
